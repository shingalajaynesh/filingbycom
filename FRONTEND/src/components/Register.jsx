...
export default function Register({ onRegistered }) {
  ...
  const handleContinue = (event) => {
    ...
    if (!firstName.trim() || !lastName.trim() || !email.trim()) {
      setError("Enter your first name, last name, and email to continue.");
      return;
    }
  ...
  const handleRegister = (event) => {
    ...
    if (!mobile.trim() || !password.trim()) {
      setError("Enter your mobile number and password to finish registration.");
      return;
    }
              ...
              <div className="sm:block">
                ...
                >
                  <svg aria-hidden="true" viewBox="0 0 48 48" className="h-5 w-5">
                    <path
                      fill="#FFC107"
                      d="M43.611 20.083H42V20H24v8h11.303C33.655 32.659 29.281 36 24 36c-6.627 0-12-5.373-12-12s5.373-12 12-12c3.059 0 5.842 1.154 7.961 3.039l5.657-5.657C34.988 6.053 29.838 4 24 4 12.955 4 4 12.955 4 24s8.955 20 20 20 20-8.955 20-20c0-1.341-.138-2.651-.389-3.917z"
                    />
                    <path
                      fill="#FF3D00"
                      d="M6.306 14.691l6.571 4.819C14.655 16.108 18.961 12 24 12c3.059 0 5.842 1.154 7.961 3.039l5.657-5.657C34.988 6.053 29.838 4 24 4 16.318 4 9.656 8.337 6.306 14.691z"
                    />
                    <path
                      fill="#4CAF50"
                      d="M24 44c5.632 0 10.715-2.148 14.577-5.639l-6.726-5.697C29.823 34.27 27.108 35.2 24 35.2c-5.257 0-9.615-3.291-11.293-7.945l-6.52 5.026C9.495 39.192 16.028 44 24 44z"
                    />
                    <path
                      fill="#1976D2"
              ...
              <div className="rounded-[2.25rem] border border-white/10 bg-white/10 p-6 shadow-[0_35px_80px_rgba(0,0,0,0.25)] backdrop-blur-sm">
                <div className="rounded-[1.75rem] bg-white p-5">
                  <img
                    src="/logo.jpeg"
                    alt="Company logo"
                    className="h-56 w-56 rounded-[1.25rem] object-contain sm:h-64 sm:w-64"
                  />
                </div>
              </div>
            </div>
